...
# Epsilon wall function structure and constructor
struct EpsilonWallFunction{I,V} <: AbstractWallFunction
    ID::I 
    value::V 
end
Adapt.@adapt_structure EpsilonWallFunction
EpsilonWallFunction(name::Symbol; kappa=0.433, beta1=0.075, cmu=0.09, B=5.2, E=9.8) = begin
    yPlusLam = y_plus_laminar(E, kappa)
    EpsilonWallFunction(
        name, (kappa=kappa, beta1=beta1, cmu=cmu, B=B, E=E, yPlusLam=yPlusLam)
        )
...
function fixedValue(BC::EpsilonWallFunction, ID::I, value::V) where {I<:Integer,V}
    # Exception 1: Value is scalar
    if V <: Number
        return EpsilonWallFunction{I,typeof(value)}(ID, value)
        # Exception 2: value is a tupple
    elseif V <: NamedTuple
        return EpsilonWallFunction{I,V}(ID, value)
    # Error if value is not scalar or tuple
    else
        throw("The value provided should be a scalar or a tuple")
    end
